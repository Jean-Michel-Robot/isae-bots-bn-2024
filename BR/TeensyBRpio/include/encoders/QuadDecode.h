...
extern "C" void ftm1_isr(void)
{
  ...
  {                         //read the timer overflow flag (TOF in FTM1_SC)
    FTM1_SC &= ~FTM_SC_TOF; //if set, clear overflow flag
    if (QuadDecode::instance().first_interrupt_1)
    { //When FTM is initialized, it generate an interrupt. We escape it because it is not an proper overflow
      QuadDecode::instance().first_interrupt_1 = false;
      return;
    }
    if (FTM1_CNT < 0x8000)
    { //distinguish if the overflow is by the min value or the max
...
extern "C" void ftm2_isr(void)
{
  ...
  {                         //read the timer overflow flag (TOF in FTM1_SC)
    ...
    if (QuadDecode::instance().first_interrupt_2)
    { //When FTM is initialized, it generate an interrupt. We escape it because it is not an proper overflow
