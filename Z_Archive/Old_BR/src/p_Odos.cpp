...
OdosPositionTask::OdosPositionTask(TaskType type):Task(type)
{
#ifdef ODO_HARD
...
void OdosPositionTask::setPosition(Position2D pos)
{
  m_robotPosition = pos;
...
#ifdef ODO_HARD
  m_odoLeftCount = QuadDecode.getCounter2();  // - pour la pmi
  m_odoRightCount = - QuadDecode.getCounter1();   // + pour la pmi
#elif defined(ODO_SOFT)
  odoL = - knobLeft.read();
  odoR = knobRight.read();
#endif
...
bool OdosPositionTask::isRobotBlocked(float seuil)
{ // si les odometres detectent une vitesse nulle
  m_speedOdometerL = m_filterSpeedOdoL.computeOutput(std::abs(float((m_odoLeftCount - m_oldOdoL)) / (micros() - m_microsOfLastMesureSpeedOdometers) * 1e6 / UNITS_ODOS), micros());
  m_speedOdometerR = m_filterSpeedOdoR.computeOutput(std::abs(float((m_odoRightCount - m_oldOdoR)) / (micros() - m_microsOfLastMesureSpeedOdometers) * 1e6 / UNITS_ODOS), micros());
...
#define THETA_RECAL_EPS PI/8
void OdosPositionTask::setPositionAvecRecalage()
{ // le robot a recu un ordre de recalage, et est arrive au contact du mur
  ...
  if (std::abs(modulo_pipi(machineAEtatAsservInstance->getGoalOrder().goalPos.theta - 0)) < THETA_RECAL_EPS ||
    ...
    rosTask->logPrint("recalage d'angle uniquement : theta "+ String(m_robotPosition.theta, 6) + " -> " + String(machineAEtatAsservInstance->getGoalOrder().goalPos.theta, 6));
    setPosition(Position2D(m_robotPosition.x, machineAEtatAsservInstance->getGoalOrder().goalPos.y, machineAEtatAsservInstance->getGoalOrder().goalPos.theta));
    
  return;
}

Position2D OdosPositionTask::getRobotPosition() const
{
