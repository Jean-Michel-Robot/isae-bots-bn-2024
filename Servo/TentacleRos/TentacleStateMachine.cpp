...
// getter for the tantacleId
int TentacleStateMachine::getTentacleId() {
    return tentacleId;
}

void TentacleStateMachine::setTentacleRos(TentacleRos *tentacleRos) {
    this->tentacleRos = tentacleRos;
}

// handles the actions associated with entering a state
void TentacleStateMachine::stateEnter(State state) {
    currentState = state;
    if (tentacleRos != NULL) tentacleRos->publishTentacleState(tentacleId);
        ...
        case STATE_RELEASING:
            if (!timerBeforePumpOffReleasing.isExpired(millis())) {
                if (timerLostContactBeforePumpOffReleasing.isStarted() && timerLostContactBeforePumpOffReleasing.isExpired(millis())) {
                    ...
                    if (tentacleRos != NULL) tentacleRos->publishReleaseFb(tentacleId, RELEASE_BAD); // bad deposit msg
                ...
                if (!tentacle.isSwitchBuoyPressed()) {
                    ...
                    if (tentacleRos != NULL) tentacleRos->publishReleaseFb(tentacleId, RELEASE_GOOD); // good deposit msg
                    ...
                    if (tentacleRos != NULL) tentacleRos->publishReleaseFb(this->tentacleId, RELEASE_FAIL); // failure msg
